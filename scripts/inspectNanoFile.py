...
if __name__ == '__main__':
    ...
    parser.add_option("-f", "--file", dest="fileIn", type="string", default=None, help="input NanoAOD file")
    ...
    if options.fileIn.endswith(".root"):
        filedata = FileData(inspectRootFile(options.fileIn))
    elif options.fileIn.endswith(".json"):
        filedata = FileData(json.load(open(options.fileIn,'r')))
    else: raise RuntimeError("Input file %s is not a root or json file" % options.fileIn)
